...
| **ReputaZap** | Sistema/plataforma | Hardware barato (canal) + mensalidade (motor) |
...
URL única por negócio: `reputazap.com/avaliar?place_id=XXX`
...
URL: `reputazap.com/ativar?from=web`
...
### 3. Painel do dono (`reputazap.com/app`)
...
| Email transacional | Resend (domínio reputazap.com em verificação) |
| APIs externas | Google Places API |
| Deploy | Vercel (auto-deploy via `git push origin main`) |
| Domínio | `reputazap.com` (apontado), `reputazap.com.br` (redirect comprado) |
...
| Item | Status |
|---|---|
| Domínio `reputazap.com` | ✅ Ativo |
| Domínio `reputazap.com.br` | ✅ Redirect pra .com (via Hostinger) |
...
| ImprovMX (receber contato@reputazap.com) | ⏳ Não configurado |
...
1. **Nome da marca final** (ReputaZap mantido provisoriamente)
...
- **GitHub:** github.com/ricardofiorini-ship-it/reputazap
- **Deploy:** reputazap.com (Vercel auto-deploy via main branch)
