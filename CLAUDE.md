...
**A medição do site é uma dessas proteções, e agora tem guarda no build** (01/09/2026). O commit `de1acf5` (LGPD, 22/08) arrancou as tags inline de GA4 e Pixel dos dois shells do React e **esqueceu de pôr o `/consent.js` no lugar**. `/app` e `/app-legacy` — o painel do cliente, e o `/diagnostico` redireciona pra lá — ficaram **de 22/08 a 01/09 sem medição nenhuma**, calados: os eventos do `AppV2` são todos `if (window.gtag) gtag(...)`, então sem a tag o `if` dá falso e nada dispara. No GA4 isso apareceu como **queda de tráfego**, não como defeito. O `index-v3.html` nasceu sem a linha (29/08) por ter sido copiado desses shells. Corrigido em `e3df6b2`. No mesmo dia se descobriu que o **molde** de `scripts/build-articles.mjs` ainda escrevia Pixel + GA4 inline — a rodada de LGPD consertou os artigos já publicados em `public/artigos` e não o gerador que os escreve; nenhum artigo novo saiu no intervalo, então nada vazou, mas o próximo teria ido ao ar rastreando sem aceite (`9dfb23d`). **Regra: quem carrega GA4 e Meta Pixel é o `/consent.js`, sempre, e só ele.** Pág. pública leva um (e só um — dois contam a visita em dobro); pág. legal leva com `data-prefs-only`; a `/avaliar` não leva nada. O `scripts/check-tracking.mjs` roda no `npm run build` entre os geradores e o vite e **barra o deploy** se a regra quebrar — inclusive nos moldes em `scripts/`. A lista de alvos vem **do disco**, não escolhida a dedo (foi uma lista a dedo que deixou o `/app` passar), e carrega os dois controles: `landing.html` tem que ter, `avaliar.html` tem que não ter — se um dos dois falhar a sonda está cega e o build para, mesmo que o resto pareça certo.

**Alarme que grita à toa acaba no mesmo lugar que proteção que falha calada: ninguém fica sabendo** (27/08/2026). O avesso do princípio acima, e igualmente caro. O `cron/db-health` tocava o banco **uma vez** e alertava no estouro dos 8s — às 3h UTC, sem tráfego, com a função acordando fria. Deu 8002ms: bateu no cronômetro, não achou o banco fora. O `api/r/[code].js` já resolvia isso desde 08/06 (`queryWithRetry`, 3 tentativas), porque lá confundir soluço com "placa não existe" derrubaria a placa do cliente — mas o monitor, que faz a MESMA pergunta ao MESMO banco, tinha ficado de fora da blindagem. **Regra: toda sonda que dispara ação (email, bloqueio, fallback) retenta antes de concluir, e a recuperação numa retentativa não pode virar silêncio** — soluço repetido é o aviso prévio da queda real, então vai pra log com `console.warn` + campo `retried` na resposta. Banco realmente fora falha as 3 tentativas; a vigilância não afrouxa, só a gritaria.

**A régua de volume não pode depender de consentimento** (02/09/2026). Medido: em 7 dias o servidor viu **~1.100 carregamentos de página com navegador real** (Vercel → Observability → Edge Requests → rota `/consent.js` → aba Bot Name → linha "No Bot Name"; 33 eram robô) e o **GA4 registrou 195 visualizações**. Não é bug: o banner de 22/08 sobe o GA4 negado, e a modelagem que o Google usa pra recuperar quem não aceita exige **1.000 eventos/dia por 7 dias** — o site faz 200, então **nunca liga**. O efeito é que o GA4 deixou de responder "quantos" e passou a responder "quantos consentiram", que é 5 a 10 vezes menos. Isso já custou uma leitura errada: a queda de agosto foi lida como "o tráfego caiu". **A correção não é afrouxar o consentimento — é ter uma régua que não dependa dele.** É a **catraca**: `POST /api/visitas` → tabela `page_hits`, contadores diários por página e origem, **sem IP, sem user agent, sem cookie, sem id de navegador, sem hora exata**. Não individualiza, logo não é dado pessoal (Art. 5º, I) e não há o que consentir — declarado na Política **§6.5** (versão 1.1). A chamada mora **dentro do `consent.js`** de propósito, pra herdar a garantia de presença do `check-tracking.mjs`, que agora também **barra o build se a catraca sumir**. Painel em `/admin/visitas`. Rodar `supabase/schema-visitas.sql` uma vez. **Taxa de aceite (04/09):** o mesmo endpoint conta, com a mesma natureza agregada, quantas vezes cada opção do banner foi escolhida (`consent_hits`, `supabase/schema-consentimento.sql`, Política **1.2**). Responde a única coisa acionável que sobrou — **se o aceite já for alto, reescrever o banner rende pouco** e o GA4 é magro por volume, não por texto. E serve de aferição cruzada: o `% que o GA4 enxerga` tem que bater, grosso modo, com `páginas vistas no GA4 ÷ carregamentos da catraca` — duas medições independentes do mesmo número, então divergência grande significa que **uma das duas está mentindo**. **Nunca comparar o número da catraca com o do GA4 como se medissem a mesma coisa** — e nunca acrescentar coluna que individualize, sob pena de derrubar a base legal e tornar a Política publicada falsa.
...
1. **Painel legado — DECIDIDO em 04/09/2026: fica vivo por enquanto, e morre quando o V3 substituir o `/app`.** Não mexer nele até lá, nem pra consertar os links. Contexto: são três painéis no ar — `/app` → `index-v2.html` → `AppV2.jsx` (6.664 linhas, **o que os clientes usam**), `/app-legacy` → `index.html` → `App.jsx` (2.270 linhas, **sem um único link apontando pra ele em todo o site** — só se chega digitando), e `/painel-f7dsaz3c` → `index-v3.html` → `src/v3/` (em construção). Matar o legado é apagar arquivo e rota: **nenhum dado passa por ali**, é a mesma conta e o mesmo banco vistos por outra tela. Ele sobrevive como seguro enquanto o V3 não assume. O que ainda tem de errado lá: `src/App.jsx:1436-1437, 1876` apontam pro **SKU placeholder do Mercado Livre**, e os três pro **mesmo anúncio** (quem clica em "Cartão NFC" cai numa placa de acrílico) — é a única tela que ainda manda cliente pra fora do site. O `/app` e o V3 já vendem pelo checkout próprio (`/kit?add=…` → Mercado Pago).
...
**Formato do código — a letra do produto viaja dentro do código (desde 01/09/2026).** `STAR-C9K4T7`: **C**=cartão, **B**=placa de balcão, **M**=placa de mesa, **P**=pulseira. Mapa em `api/_lib/plates.js` (`PRODUCT_LETTER`), que é também a **fonte única** dos tipos válidos (`PRODUCT_TYPES`, importado por `api/plates.js`) — produto sem letra não vira lote, e o erro é alto, não silencioso. **É o COMPRIMENTO que separa as eras:** código novo tem **6** caracteres depois do hífen (letra + 5), código legado tem **5** e ali a primeira letra é sorteada e **não significa nada** — nunca ler a letra de um código de 5. Códigos já impressos não mudam. Nenhuma rota valida formato (busca é `trim`+`toUpperCase`+match exato), então as duas eras convivem. **Por que existe:** até `e228903` (23/06/2026) o "Cartão NFC" não era opção na tela de produção, então todo cartão feito antes disso foi cadastrado como placa — `plates.product_type` e `production_batches.product_type` erram juntos (o `handleCreateBatch` grava o mesmo valor nos dois) e a única pista da verdade é o `batch_name` digitado à mão. Qualquer análise por `product_type` anterior a 23/06 é inválida; conserto por lote em `supabase/lotes-para-conferir.sql`.
