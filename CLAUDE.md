...
**A lista de alvos de uma verificação deriva dos ARQUIVOS ALTERADOS, nunca de páginas escolhidas a dedo.** Controle negativo prova que a sonda enxerga; **não** prova que a lista está completa. Caso que originou (22/08/2026, item 4 da LGPD): quatro páginas verificadas em `public/`, todas limpas — mas `index.html` e `index-v2.html` vivem na **raiz** e ficaram fora do `git add`, então `/app` e `/app-legacy` seguiram rastreando sem consentimento. As duas coisas que faltavam eram a mesma: o `git add` por diretório e a sonda por amostra.
