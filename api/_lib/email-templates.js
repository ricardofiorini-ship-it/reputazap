...
// ─────────────────────────────────────────────────────────────
// DICA DA SEMANA — e-mail educativo. O conteúdo vem da FONTE ÚNICA
// (api/_lib/tips-content.js), a mesma que gera os artigos do site.
// Renderiza: badge do tema + título + parágrafos + CTA + link do
// artigo (compartilhável) + indicação no WhatsApp.
// ─────────────────────────────────────────────────────────────
export function weeklyTipEmail({ tip, unsubUrl, articleUrl }) {
  const t = tip || {};
  const isIA = t.tag === "IA";
  const accent = isIA ? "#6D28D9" : "#1A73E8";
  const badgeBg = isIA ? "#F3EEFF" : "#EAF2FE";
  const badgeText = themeLabel(t.tag);

  // CTA do e-mail precisa de URL absoluta (cliente de e-mail não resolve "/…").
  const ctaUrl = t.cta && t.cta.url
    ? (t.cta.url.startsWith("/") ? "https://startouch.com.br" + t.cta.url : t.cta.url)
    : null;

  const paras = (t.paragraphs || []).map((p) =>
    `<p style="font-size:15px;color:#3c4043;line-height:1.7;margin:0 0 16px;">${escapeHtml(p)}</p>`
  ).join("\n        ");

  const artLink = articleUrl
    ? `<p style="font-size:13px;color:#5F6368;line-height:1.6;margin:16px 0 0;">Prefere ler no site — ou compartilhar com alguém? <a href="https://startouch.com.br${articleUrl}" style="color:#1A73E8;text-decoration:none;font-weight:600;">Abrir esta dica →</a></p>`
    : "";

  return {
    subject: t.headline || "Dica da semana",
    html: shell({
      ...
      body: `
        ...
        <h1 style="margin:0 0 16px;font-size:23px;color:#202124;line-height:1.3;">${escapeHtml(t.headline || "")}</h1>

        ${paras}

        ${ctaUrl ? cta(ctaUrl, t.cta.text, accent) : ""}
        ${artLink}

        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#fff;border:1px solid #e5e7eb;border-radius:12px;margin:22px 0 4px;">
        ...
        <p style="font-size:13px;color:#5F6368;line-height:1.6;margin:18px 0 0;">Uma dica curta a cada poucos dias, do time StarTouch — pra você aparecer mais no Google e nas IAs sem complicação.</p>
      `
    })
  };
}
...
// ─────────────────────────────────────────────────────────────
// PRÉVIA DA DICA (só pro admin, 1 dia antes) — com botão de aprovar.
// Mostra exatamente o que os clientes vão receber + banner de aprovação.
// ─────────────────────────────────────────────────────────────
export function tipPreviewEmail({ tip, approveUrl, articleUrl, sendDate }) {
  const t = tip || {};
  const isIA = t.tag === "IA";
  const accent = isIA ? "#6D28D9" : "#1A73E8";
  const badgeBg = isIA ? "#F3EEFF" : "#EAF2FE";
  ...
  const dataFmt = sendDate ? sendDate.split("-").reverse().join("/") : "amanhã";

  return {
    subject: `👀 Prévia da dica — aprove pra disparar (${dataFmt})`,
    html: shell({
      title: "👀 PRÉVIA — SÓ VOCÊ RECEBE ISSO",
      headerColor: "#B7791F",
      body: `
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#FFF8E1;border:1px solid #FCE8A6;border-radius:12px;margin:0 0 20px;">
          <tr><td style="padding:16px 18px;">
            <div style="font-size:15px;font-weight:800;color:#202124;margin-bottom:4px;">Esta dica sai pros clientes em ${escapeHtml(dataFmt)}, às 9h.</div>
            <div style="font-size:13.5px;color:#5F6368;line-height:1.55;margin-bottom:14px;">Ela só dispara se você aprovar. Quer mudar alguma coisa? É só me avisar no chat que eu ajusto e te reenvio a prévia.</div>
            <table role="presentation" cellspacing="0" cellpadding="0"><tr><td style="border-radius:10px;background:#137333;">
              <a href="${approveUrl}" style="display:inline-block;padding:13px 26px;font-size:15px;font-weight:800;color:#fff;text-decoration:none;border-radius:10px;font-family:Arial,sans-serif;">✅ Aprovar e disparar</a>
            </td></tr></table>
          </td></tr>
        </table>

        <p style="font-size:11px;font-weight:700;color:#80868B;text-transform:uppercase;letter-spacing:0.05em;margin:0 0 14px;">↓ Como o cliente vai ver ↓</p>

        <div style="display:inline-block;font-size:11px;font-weight:700;letter-spacing:0.05em;text-transform:uppercase;color:${accent};background:${badgeBg};border-radius:999px;padding:5px 12px;margin:0 0 12px;">${badgeText}</div>
        <h1 style="margin:0 0 16px;font-size:23px;color:#202124;line-height:1.3;">${escapeHtml(t.headline || "")}</h1>
        ${paras}
        ${articleUrl ? `<p style="font-size:13px;color:#5F6368;margin:14px 0 0;">Vira o artigo: <a href="https://startouch.com.br${articleUrl}" style="color:#1A73E8;text-decoration:none;font-weight:600;">${escapeHtml(articleUrl)}</a></p>` : ""}
      `
    })
  };
}
